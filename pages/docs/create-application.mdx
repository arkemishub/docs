    ...
    start it, follow each tabs to init projects:
     <Tabs items={['Backend', 'Console', 'Frontend']}>
       <Tabs.Tab>
           ...
           #### Init arke_system project
           Init `arke_system` database schema to store system's and common data.
           <Tabs items={['Mix','Arkectl']}>
               <Tabs.Tab>
                   ```bash
           ...
           #### Create your project
           Init your `arke_project` database schema with an identifier that will be used from your client application

           <Callout type="error">
               Avoid special characters and use `_` for spaces, this ID will be used as database schema name. Also `-`
               is not recommended
           </Callout>
           ...
           #### Create your super admin member
           To create and manage your arke you need an authenticated user, to create the first super admin member on your
           project you can use:

           ```bash
           mix arke_postgres.create_member --username admin --password admin --project <PROJECT_ID>
           ```

import {Callout, Tabs, Steps, FileTree, Tab} from 'nextra/components'

# Create application
We recommend starting a new Arke app using **create-app** through the arkectl, which sets up everything automatically for you.
if you haven't installed it yet follow the [arkectl guide](/docs/command-line-tools).
...
<Tabs items={['Locally', 'Not locally']}>
  ...
  <Tabs.Tab>
    ...
    <Callout type="warning">
       [Docker](https://docs.docker.com/get-docker/) must be installed and running for project installation
    </Callout>
     ...
     <Tabs items={['Backend', 'Console', 'Frontend']}>
       <Tabs.Tab>
           ...
           <Tabs items={['MacOS', 'Windows']}>
               ...
               <Tabs.Tab>
                   Add a `env.bat` file in the root of the project to set the Database env variables:

                   <Callout type="warning">
                       Not use quotes on env variable definition
                   </Callout>
           ...
           <Callout type="info">
               If you want to export data from another DB you can use
               `mix arke.export_data --project {PROJECT_ID} --splitfile`. Move this file under the `registry` folder
               and run the seed_project command.
           </Callout>
           ...
           <Callout type="info">
               If you did not set a username and a password, the default are username=admin password=admin
               Run `mix help arke_postgres.create_member` to get other options
           </Callout>
       ...
       <Tabs.Tab>
           ...
           NEXTAUTH_SECRET={GENERATED_NEXT_AUTH_SECRET}
           NEXT_PUBLIC_ARKE_PROJECT={PROJECT_ID}
           ```
